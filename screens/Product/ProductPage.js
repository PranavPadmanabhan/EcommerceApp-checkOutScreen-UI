...
import { height,images} from '../../constants/constants'
...
function ProductPage() {
    ...
    const bottomSheetInterpolate = alignment.interpolate({
     ...
     const opacityInterpolation = alignment.interpolate({
          inputRange:[0,1], 
          outputRange:[0 , 1]
      })
  ...
  return (
    <View style={styles.Container}>
        <Animated.View style={[styles.overlay,{opacity:opacityInterpolation}]} />
        <Header />
        ...
        <Animated.View style={[styles.sheet,bottomsheetStyle,{backgroundColor:colorInterpolation}]} >
            {
                sheetVisible ? (
                    <Text onPress={bottomsheetClose} style={{color:"black"}}>close</Text>

                ):(
                    <Text onPress={bottomsheetOpen} style={{color:"white"}}>Buy now</Text>
